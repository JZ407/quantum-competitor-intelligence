...
# Search /news up to 200
r, best = find_exact_max('/news', get_news_ids, '/news', 30, 200)
results.append(r)
results.append('')

# If still has content, search higher
if best is None:
    r, best = find_exact_max('/news', get_news_ids, '/news', 200, 500)
    results.append(r)
    results.append('')

# Search /reference up to 200
r, best = find_exact_max('/reference', get_reference_ids, '/reference', 30, 200)
results.append(r)
results.append('')

with open('probe_max_pages3_report.txt', 'w', encoding='utf-8') as f:
    f.write('\n'.join(results))
print('Saved to probe_max_pages3_report.txt')
